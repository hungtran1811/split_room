export function toWholeVnd(value: unknown): number {
...
type AmountEntry = { id?: string; amount?: number };

export function allocateWholeAmounts(
  entries: AmountEntry[] = [],
  targetTotal: number | null = null,
  ...
  const prepared = entries
    ...
    .filter((entry): entry is { id: string; amount: number; index: number } =>
      Boolean(entry.id),
    );
...
export function buildWholeEqualShares(
  total: number,
  ids: string[] = [],
): Record<string, number> {
  const memberIds = [...ids];
...
export function normalizeWholeBalances(
  memberIds: string[] = [],
...
export function normalizeWholeMatrix(
  memberIds: string[] = [],
